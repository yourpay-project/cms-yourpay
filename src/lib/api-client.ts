...
export function setTokensInCookies(access: string, refresh?: string, accessMaxAgeSeconds?: number): void {
...
async function requestWithRefresh<T>(
  ...
  if (!res.ok) {
    const message = typeof data === "object" && data !== null && "message" in data ? String((data as { message?: string }).message) : res.statusText;
    throw new ApiClientError(message, res.status, typeof data === "object" && data !== null && "code" in data ? String((data as { code?: string }).code) : undefined);
  }
...
export const apiClient = {
  get: <T>(path: string, init?: RequestOptions) => apiRequest<T>(path, { ...init, method: "GET" } as RequestOptions),
  ...
  delete: <T>(path: string, init?: RequestOptions) => apiRequest<T>(path, { ...init, method: "DELETE" } as RequestOptions),
};
