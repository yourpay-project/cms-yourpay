...
import { useCustomerDevicesQuery } from "../model";
import { formatDateTime, formatDeviceTitle, formatOperatingSystem } from "../lib";
...
interface ViewDevicesDialogProps {
  open: boolean;
 ...
 * @returns Devices modal content.
 */
const ViewDevicesDialog: FC<ViewDevicesDialogProps> = ({ open, onOpenChange, customerId }) => {
          ...
          query.data!.map((device) => {
            const fields: UserDetailFieldItem[] = [
              ...
              { label: "App Version", value: device.appVersion },
              {
                label: "Location",
                value:
                  device.geoLat != null && device.geoLng != null
                    ? `${device.geoLat}, ${device.geoLng}`
                    : "-",
              },
            ];
...
export default ViewDevicesDialog;
